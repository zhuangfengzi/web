...
window.ALIYUN_CONFIG = {
    ...
    decodeAppIdFromVersion: function(version) {
        try {
            console.log('尝试解码版本号:', version);
            // 解析版本号格式: major.minor.encodedAppId
            const parts = version.split('.');
            if (parts.length !== 3) {
                console.warn('版本号格式错误，跳过解码');
                return null;
            }
            
            const encodedAppId = parts[2];
            if (!encodedAppId || encodedAppId.length < 8) {
                console.warn('编码的AppID太短，跳过解码');
                return null;
            }
            
            // 恢复Base64的等号填充
            let base64String = encodedAppId;
            while (base64String.length % 4 !== 0) {
                base64String += '=';
            }
            
            // 解码Base64
            const decodedAppId = atob(base64String);
            if (decodedAppId && decodedAppId.length >= 8) {
                console.log('成功解码AppID:', decodedAppId);
                return decodedAppId;
            }
            
            console.warn('解码后的AppID无效');
            return null;
            
        } catch (error) {
            console.warn('解码AppID失败:', error.message);
            return null;
        }
    },
            ...
            overlay.innerHTML = `
                ...
                ">
                    ...
                    <p style="margin-bottom: 20px; line-height: 1.6; opacity: 0.9;">
                        ...
                        <small style="color: #f39c12;">请在Flutter应用的设置对话框中配置AppID</small>
                    </p>
