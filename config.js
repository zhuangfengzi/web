    ...
    getToken: function() {
        try {
            ...
            if (urlToken && urlToken.trim()) {
                console.log('使用URL传递的访问凭证');
                // 保存到本地存储以供下次使用
                this.saveTokenToStorage(urlToken);
                return urlToken;
            }
    ...
    decodeAppIdFromVersion: function(version) {
        try {
            ...
            if (decodedData && decodedData.length >= 8) {
                console.log('成功解析应用标识:', decodedData.substring(0, 8) + '***');
                return decodedData;
            }
            ...
            console.warn('应用标识解析失败:', error.message);
            return null;
        }
    },
